...
public class Line extends Shape {
	...
	public double getAngle() {
		// relative to point1
		if (Apollonius.eq(this.point1.x, this.point2.x)) {
	...
	public boolean intersectsWithPoint(Point p) {
		if (Apollonius.eq(this.point1.x, this.point2.x)) {
			return Apollonius.eq(p.x, this.point1.x);
		}
		else {
			double y = this.getGradient() * p.x + this.getYIntersept();		// f(x) = mx + b
			return Apollonius.eq(p.y, y);		// ob f(x) = p.y
		}
	}

	public IntersectionCount intersectsWithLine(Line line) {
		if (Apollonius.eq(this.point1.x, this.point2.x) && Apollonius.eq(line.point1.x, line.point2.x)) { // if both are a vertical line
			if (Apollonius.eq(this.point1.x, line.point1.x)) {
		...
		else if (Apollonius.eq(this.point1.x, this.point2.x) || Apollonius.eq(line.point1.x, line.point2.x)) { // if either one is a vertical line
			...
			if (Apollonius.eq(m1, m2)) {
				if (Apollonius.eq(b1, b2)) {
					// invariant
					return IntersectionCount.INFINITY;
				}
				// parallel
	...
	public Point intersectionPointWithLine(Line line) {
		if (Apollonius.eq(this.point1.x, this.point2.x)) {
			...
			if (Apollonius.eq(line.point1.x, line.point2.x)) {
		...
		else if (Apollonius.eq(line.point1.x, line.point2.x)) {		
			...
			if (Apollonius.eq(m1, m2)) { // parallel or invariant
				System.out.println("Lines don't intersect or are invariant");
				return null; //new Point(0,0);
			}
			else {
	...
	public Boolean pointIsOnLeftSide(Point point) {
		...
		else if(Apollonius.eq(this.point1.y, this.point2.y)) {
			// horizontal line
